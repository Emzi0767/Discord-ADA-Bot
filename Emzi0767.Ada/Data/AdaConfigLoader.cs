...
namespace Emzi0767.Ada.Data
{
    ...
    /// </summary>
    public sealed class AdaConfigLoader
    {
        /// <summary>
        ...
        public async Task<AdaConfig> LoadConfigurationAsync(FileInfo file)
        {
            ...
            using (var sr = new StreamReader(fs, AdaUtilities.UTF8))
                json = await sr.ReadToEndAsync().ConfigureAwait(false);

            // deserialize the config
            return JsonConvert.DeserializeObject<AdaConfig>(json);
        }
        ...
        /// <returns></returns>
        public async Task SaveConfigurationAsync(AdaConfig config, FileInfo file)
        {
            ...
            using (var sr = new StreamWriter(fs, AdaUtilities.UTF8))
            {
        ...
        public void ValidateConfiguration(AdaConfig config)
        {
            // validate the config
            if (config == null || config.Version == null || config.Discord == null || config.PostgreSQL == null || config.Bot == null)
