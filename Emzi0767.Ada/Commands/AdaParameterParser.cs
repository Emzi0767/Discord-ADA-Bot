...
// https://github.com/RogueException/Discord.Net/blob/dev/src/Discord.Net.Commands/PrimitiveParsers.cs

namespace Emzi0767.Ada.Commands
{
    public delegate bool TryParseDelegate<T>(string value, out T result);
    public delegate bool ContextParseDelegate<T>(AdaCommandContext ctx, string value, out T result);

    public class AdaParameterParser
    {
        private Dictionary<Type, Delegate> Parsers { get; set; }

        public AdaParameterParser()
        {
